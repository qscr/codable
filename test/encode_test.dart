    ...
    test("Can encode map of Coding objects", () {
      final out = encode((object) {
        object.encodeObject(
          ...
          Parent(
            "Bob",
            childMap: {"fred": Child("Fred"), "null": null, "sally": Child("Sally")},
          ),
        );
      });
  ...
  group("Coding object references", () {
    ...
    test("If reference doesn't exist in objectMap, an error is thrown when creating document", () {
      final container = Container(
        Parent(
          "Bob",
          childMap: {"c": Child._()..referenceURI = Uri(path: "/definitions/child")},
        ),
        {},
      );
    ...
    test("Parent can contain reference to child in a list of objects", () {
      final container = Container(
        Parent(
          "Bob",
          children: [Child("Sally"), Child._()..referenceURI = Uri(path: "/definitions/child")],
        ),
        {"child": Child("Fred")},
      );
